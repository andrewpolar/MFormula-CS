    ...
    class PLL
    {
        ...
        public void Update(double x, double delta, double mu)
        {
            if (x < _xmin)
            {
                ...
                _xmin = x;
                Console.WriteLine("xmin is corrected");
            }
            ...
            if (x > _xmax)
            {
                ...
                Console.WriteLine("xmax is corrected");
            }
            ...
            double deltaUpper = delta * leftx / _deltax * mu;
            double deltaLower = delta * rightx / _deltax * mu;
            _y[left + 1] += deltaUpper;
            _y[left] += deltaLower;

            for (int i = left + 2; i < _y.Length; ++i)
            {
                deltaUpper *= _decay;
                _y[i] += deltaUpper;
            }
